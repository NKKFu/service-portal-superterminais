<template>
  <div class="page-container glass-panel">
    <div class="page-header">
      <h2>Editar Empresa #{{ route.params.id }}</h2>
    </div>

    <div v-if="loadingInit" class="state-container">
      Carregando detalhes...
    </div>
    ...
    <form v-else @submit.prevent="handleSubmit" class="registration-form">
      ...
      <div class="file-upload-section">
        <label class="file-upload-label">Documento da Empresa</label>
        <div
          class="file-drop-zone"
          :class="{ 'has-file': selectedFile || currentFileName }"
        ...
        >
          ...
          />
          <div v-if="!selectedFile && !currentFileName" class="drop-hint">
            ...
            <p>Clique ou arraste um arquivo para substituir</p>
          ...
          <div v-else class="file-preview">
            ...
            <div class="file-info">
              <span class="file-name">{{ selectedFile ? selectedFile.name : currentFileName }}</span>
              <span class="file-size" v-if="selectedFile">{{ formatFileSize(selectedFile.size) }}</span>
              <span class="file-size" v-else>Arquivo atual</span>
            </div>
            <button v-if="selectedFile" type="button" class="remove-file-btn" @click.stop="clearFile">
...
const handleCancel = () => {
  if (window.history.length > 1) {
    ...
    router.push(isInternal.value ? `/companies/${route.params.id}` : '/');
  }
};
    ...
    if (response.ok) {
      ...
      if (company) {
        form.value = {
          ...
          cpf: company.cpf ? maskCPF(company.cpf) : '',
          foreignDocument: company.foreignDocument || '',
          nickname: company.nickname || '',
          profileType: company.profileType || 'NEW_USER',
          companyIncome: company.companyIncome || 0,
          documentUrl: company.documentUrl || ''
        };
        currentFileName.value = company.documentFileName || null;
        if (form.value.cnpj) handleCnpjInput();
        if (form.value.cpf) handleCpfInput();
      } else {
        error('Empresa não encontrada.');
        router.push('/companies');
      }
    } else {
      error('Falha ao buscar detalhes da empresa.');
      router.push('/companies');
    }
  } catch(err) {
    console.error(err);
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const msg = result.message || 'Erro ao atualizar empresa.';
